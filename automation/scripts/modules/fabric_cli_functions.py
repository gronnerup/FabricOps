...
def get_item(item_path: str, retry_count: int = 0):
    for attempt in range(retry_count + 1):
        try:
            cli_response = run_command(f"get {item_path} -f -q .")
...
def get_item_id(item_path: str, retry_count: int = 0):
    for attempt in range(retry_count + 1):
        try:
            cli_response = run_command(f"get {item_path} -f -q id")
...
def get_connection(connection_identifier):
    if is_guid(connection_identifier): 
        ...
        return json.loads(run_command(f"get .connections/{connection_identifier}.Connection -f -q ."))
