...
if env_definition:
    ...
    for layer, layer_definition in layers.items():
        if layer.lower() in layers_to_deploy:        
            ...
            workspace_id = fabcli.run_command(f"get '{workspace_name_escaped}.Workspace' -f -q id").strip()
            ...
            try:
                ...
                if bindings:
                    ...
                    for binding in bindings:
                        ...
                        lakehouse_layer = binding.get("lakehouse_layer")
                        semantic_models = binding.get("semantic_models", [])

                        # Resolve lakehouse connection and SQL endpoint information
                        connection_name_template = misc.get_lakehouse_connection_template(env_definition, lakehouse_layer, lakehouse_name)
                        ...
                        for semantic_model_name in semantic_models:
                            try:
                                if not fabcli.item_exists(f'{workspace_name}.Workspace/{semantic_model_name}.SemanticModel'):
                                    misc.print_warning(f"Semantic model '{semantic_model_name}' not found in workspace {workspace_name}. Skip binding.")
                                    continue

                                semantic_model_id = fabcli.run_command(f"get '/{workspace_name}.Workspace/{semantic_model_name}.SemanticModel' -f -q id").strip()
                                ...
                                misc.print_warning(f"Binding '{semantic_model_name}' to SQL endpoint for lakehouse '{lakehouse_name}' failed done.")
